...
export class OptionPricer {
  ...
  getPremium(params) {
    ...
    const rawPrice = this.blackScholesSimple.getPrice(
      params.isCall,
      params.expiry,
      timeToExpirySeconds,
      volatility,
  ...
  adjustPriceByUtilization(params, price, isBuy) {
    ...
    utilizationAfter = utilizationAfter.gte(0) ? 
      UNIT.minus(utilizationAfter.times(UNIT).div(params.equity)) : 
      UNIT;

    let utilizationAdjust;
    if (utilization.lt(params.priceRatioUtilization) && utilizationAfter.gt(params.priceRatioUtilization)) {
      const area1 = this.getArea(new BigNumber(0), new BigNumber(0), 
        params.priceRatioUtilization, params.priceRatio, 
        utilization.plus(params.priceRatioUtilization).div(2), 
        params.priceRatioUtilization.minus(utilization));

      const area2 = this.getArea(params.priceRatioUtilization, params.priceRatio,
        UNIT, params.priceRatio2,
        params.priceRatioUtilization.plus(utilizationAfter).div(2),
        utilizationAfter.minus(params.priceRatioUtilization));
      ...
      if (utilization.gte(params.priceRatioUtilization)) {
        utilizationAdjust = this.getY(params.priceRatioUtilization, params.priceRatio,
          UNIT, params.priceRatio2, utilization);
      } else {
        utilizationAdjust = this.getY(new BigNumber(0), new BigNumber(0),
          params.priceRatioUtilization, params.priceRatio, utilization);
      }
    }
